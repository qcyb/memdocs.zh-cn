...
| 问题 | Answer |
| --- | --- |
| 在合作伙伴中心，是否需要随每个设备文件上传一起提供租户 ID？ 是否需要允许企业客户访问其设备 Microsoft Store for Business (MSfB) ？     | 不是。 提供租户 ID 是合作伙伴中心中的一次性条目，可以在将来的设备上传时重复使用。 |
...
|  是否必须变成 CSP 才能参与 Windows Autopilot？ | 大多数情况下，Oem 不能使用 OEM 直接 API。  选择使用 MPC 注册设备的所有其他人必须变成 Csp 才能访问 MPC。  |
...
| 问题 | Answer |
| --- | --- |
| 必须由 OEM 提交的每个硬件哈希都包含 SMBIOS UUID (全局唯一标识符) 、MAC (媒体访问控制) 地址和唯一磁盘序列号 (如果使用 Windows 10 OEM 激活3.0 工具) ？    | 是的。 由于 Windows Autopilot 基于唯一标识应用于云配置的设备的功能，因此提交满足所述要求的硬件哈希至关重要。   |
...
|   (网络接口控制器) 和磁盘的 NIC 的部件更换和修复是什么想法？ 硬件哈希是否会无效？   |  是的。  如果替换部分，则需要收集新的硬件哈希，但它依赖于所替换的内容和部件的特征。 例如，如果你替换 TPM 或主板，则它是一个新设备，你必须具有新的硬件哈希。 如果替换一个网卡，则它可能不是新设备，并且设备将使用旧的硬件哈希。  但是，最佳做法是，应假定旧的硬件哈希无效，并在硬件更改后获取新的硬件哈希。 替换部件时建议使用此建议。 |
...
| 在 SMBIOS 表中需要满足 Windows Autopilot 硬件哈希的要求是什么？    | 它必须满足所有 Windows 10 硬件要求。  可在[此处](https://msdn.microsoft.com/library/jj128256(v=vs.85).aspx)找到更多详细信息。    |
| 如果 SMBIOS 支持 UUID 和序列号，则是否足以让 OA3 工具生成硬件哈希？    | 不是。  以下 SMBIOS 字段至少需要使用唯一值填充： ProductKeyID SmbiosSystemManufacturer SmbiosSystemProductName SmbiosSystemSerialNumber SmbiosSkuNumber SmbiosSystemFamily MacAddress SmbiosUuid DiskSerialNumber TPM EkPub |
...
| 如果管理员对现有配置文件进行更改，则所做的更改将在已分配到已部署的配置文件的设备上生效？ |不是。 Windows Autopilot 配置文件不在设备上。 它们是在 OOBE 期间下载的，应用时定义的设置。 然后，设备上会丢弃该配置文件。 如果设备已重置映像或重置，则新的配置文件设置将在下一次设备进入 OOBE 时生效。|
...
| Intune 是否可以支持 Win32 应用预安装？  | 是的。  从 Windows 10 年10月更新 (版本 1809) 开始，Intune 支持使用 .msi (和 .msix) 包装的 Win32 应用。  |
...
| 必须使用 Microsoft Endpoint Configuration Manager for Windows Autopilot  |  不是。  上面所述的共同管理 () 是可选的。 |
...
|MBR 或映像重新安装后，Windows Autopilot 是否正常工作？|是的。|
| 多次使用重置映像的计算机是否会经历 Autopilot？ 错误消息 "此用户无权注册" 是什么意思？ 错误代码801c0003。 |特定 Azure AD 用户可以在 Azure AD 中注册的设备数有限制，以及 Intune 中每个用户支持的设备数。   (这些都是可配置的，但并不是无限的。 ) 如果你重复使用设备，或者即使你回滚到以前的虚拟机快照，则会经常遇到这种情况。|
|如果设备已注册到恶意代理，会发生什么情况？                                                   |按照设计，Windows Autopilot 不会应用配置文件，直到用户使用 Azure AD 登录过程通过配置的配置文件的匹配租户登录。 发生的情况如下所示。  如果 badguys.com 注册了 contoso.com 拥有的设备，则会将该用户定向到 badguys.com。 用户输入其电子邮件/密码时，会通过 Azure AD 重定向到正确的 Azure AD 身份验证，然后系统会提示用户登录到 contoso.com。 由于 contoso.com 不会将 badguys.com 与租户匹配，因此将不会应用 Windows Autopilot 配置文件，并且将发生常规 Azure AD OOBE。|
...
|为什么 Windows Autopilot 数据存储在美国而不是主权云中？|它不是我们所存储的客户数据，而是允许 Microsoft 提供服务的业务数据，因此，数据可以在美国。 客户可以随时停止订阅服务，在这种情况下，Microsoft 将删除业务数据。|
...
| 术语 | 含义 |
| --- | --- |
| CSV  | 逗号分隔值 (类似于 Excel 电子表格) 的文件类型  |
