...
有关如何使用 Microsoft Intune 执行此操作的说明，请参阅[创建 Autopilot 部署配置文件](https://docs.microsoft.com/intune/enrollment-autopilot#create-an-autopilot-deployment-profile)文档，其中介绍了 "将所有目标设备转换为 Autopilot" 设置。 
...
Microsoft 端点 Configuration Manager 自动收集现有 Windows 10 设备的硬件哈希。 有关详细信息，请参阅[从 Windows Autopilot 的 Configuration Manager 中收集信息](https://docs.microsoft.com/configmgr/comanage/how-to-prepare-win10#windows-autopilot)。 可以将哈希信息从 Configuration Manager 提取到 CSV 文件中。

> [!Note]
> 在 Intune 上上传 CSV 文件之前，请确保第一行包含设备序列号、Windows 产品 ID、硬件哈希、组标记和分配的用户。 如果 CSV 文件顶部有标题信息，请删除该标头信息。 请参阅在[Intune 中注册 Windows 设备中](https://docs.microsoft.com/intune/enrollment/enrollment-autopilot)的详细信息。
