...
1. 在“备份后脚本”页上，输入部署运行并且执行重启（必要时）后要在每个设备上运行的 PowerShell 脚本。 否则，行为与 PreScript 相同。
...
若要删除业务流程组，请选择它，然后在功能区中或从右键单击菜单中单击“删除”。 若要编辑业务流程组，请选择它，然后在功能区中或从右键单击菜单中单击“属性”。 更改以下选项卡中的设置：
  ...
  - **成员选择**：
   ...
   - **备份后脚本**：
      - 输入部署运行并且执行重启（必要时）后要在每个设备上运行的 PowerShell 脚本。 该脚本应返回值 `0` 表示成功，或返回 `3010` 表示成功并重新启动。
