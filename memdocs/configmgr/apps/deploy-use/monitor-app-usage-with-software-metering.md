...
description: 了解 Configuration Manager 软件计数中可用的操作。
ms.date: 09/20/2017
...
ms.locfileid: "81689245"
---
# <a name="software-metering-in-configuration-manager"></a>Configuration Manager 中的软件计数

适用范围：  Configuration Manager (Current Branch)

本主题包含使用 Configuration Manager 软件计数时，可能执行的所有操作的参考。
...
|Reporting Services 点。|在查看软件计数报表前，必须先配置一个 Reporting Services 点。 有关详细信息，请参阅[报表简介](../../core/servers/manage/introduction-to-reporting.md)。|
...
##  <a name="monitor-software-metering"></a>监视软件计数
 ...
 有关 Configuration Manager 中的集合的详细信息，请参阅[集合简介](../../core/clients/manage/collections/introduction-to-collections.md)。

 有关 Configuration Manager 中的查询的详细信息，请参阅[查询简介](../../core/servers/manage/introduction-to-queries.md)。
...
- 确定哪些用户定期使用特定应用
  ...
  Woodgrove Bank 的 IT 系统经理使用 Configuration Manager 中的软件计数来实现这些业务目标。 管理员执行以下操作：

- 检查是否满足软件计数的先决条件，并确认 Reporting Services 点是否已安装且能正常运行。
- 配置软件计数的默认客户端设置：<br>管理员启用软件计数，并使用每七天一次的默认数据收集计划。<br>管理员通过配置软件清单客户端设置“列出这些文件类型的清单”  ，将软件清单配置为列出有 .exe 扩展名的文件的清单。<br>管理员添加新软件计数规则“woodgrove.exe”  ，用于监视旧版应用程序。
- 等待七天，之后客户端计算机开始报告“woodgrove.exe”  可执行文件的使用数据。
- 管理员使用 Configuration Manager 报表“所有已计数软件程序的安装基础”  ，查看哪些计算机加载了应用程序 woodgrove.exe  。
- 六个月后，管理员生成报表“已安装计数程序，但在指定日期后尚未运行此程序的计算机”  ，同时指定软件计数规则以及六个月前的一个日期。 此报表识别了在过去六个月内未运行该程序的 120 台计算机。
- 管理员进行进一步检查，以确认标识的计算机是否不再需要旧版应用程序。 然后，管理员从这些计算机中卸载旧版应用程序和 Word 2003 副本。<br>管理员生成报表“已运行特定已计数软件程序的用户”  ，为支持人员提供继续使用旧版应用程序的用户列表。
- 管理员继续每周检查一次软件计数报表，并在必要时采取修正措施。
