...
2. 在“标准程序”  页上，指定下列信息：  
    ...
    - **运行**：指定程序在客户端计算机上的运行模式。 选择下列选项之一：  
        ...
        - **常规**：程序根据系统和程序默认设置以正常模式运行。 此模式为默认模式。  
    ...
    - **驱动器模式**：指定有关此程序在网络上如何运行的信息。 选择下列选项之一：  
        ...
        - **以 UNC 名称运行**：指示程序以通用命名约定 (UNC) 名称运行。 此设置为默认设置。  

        - **需要驱动器号**：指示程序需要驱动器号来完全限定其位置。 对于此设置，Configuration Manager 可以使用客户端上任何可用的驱动器号。  

        - **需要特定驱动器号**：指定程序需要所指定的特定驱动器号来完全限定其位置。 例如，Z:  。 如果客户端已在使用指定的驱动器号，则该程序不会运行。  
...
2. 在“设备程序”  页上，指定以下设置：  
    ...
    - **从此文件夹中运行命令行**：选择此选项以指定从中运行程序的其他文件夹。  
...
6. 在“计划”页面上，配置将此包和程序部署到客户端设备的时间  。  
    ...
    对于“必需”部署，则可通过“重新运行行为”下拉菜单来配置程序的重新运行行为   。 选择从以下选项：  
...
7. 在“用户体验”  页上，指定下列信息：  
    ...
    - **系统重启(如果要求完成安装)** ：如果软件安装要求重启设备才能完成，则允许在任何已配置维护时段外执行此操作。  
    ...
    - **Embedded 设备**：将包和程序部署到启用了写入筛选器的 Windows Embedded 设备时，可以指定将包和程序安装在临时覆盖区上并稍后提交更改。 或者，在安装截止时间或在维护时段内提交更改。 如果在安装截止时间或维护时段内提交更改，则需要重新启动，且更改将保留在设备上。  
        ...
        > [!NOTE]  
        > 在包或程序部署到 Windows Embedded 设备时，请确保设备是已配置的维护时段的集合的成员。 有关向 Windows Embedded 设备部署包和程序时如何使用维护时段的详细信息，请参阅[创建 Windows Embedded 应用程序](../get-started/creating-windows-embedded-applications.md)。  
...
8. 在“分发点”  页上，指定下列信息：  
    ...
    - **允许客户端使用来自默认站点边界组的分发点**：当前或相邻边界组中的分发点中的内容不可用时，启用此选项以尝试使用站点默认边界组中的分发点。
