...
2. 从列表中选择要部署的应用程序或应用程序组。 在功能区上，单击“部署”  。  
...
在“内容”页上，单击“添加”以将此应用程序的内容分发到分发点或分发点组   。
...
例如，需要部署新的业务线应用程序。 所有用户都需要在一定时间内安装它，但希望向他们提供提前加入的选项。 还需要确保站点已将内容分发到所有分发点。 计划从今天起五天内提供应用程序。 此计划可提供时间来分发内容并确认其状态。 然后，从今天开始设置每个月的安装截止时间。 在五天内可用时，用户将在软件中心看到应用程序。 如果不执行任何操作，客户端会在安装截止时间内自动安装应用程序。
...
<!--1358147-->
从版本 1806 开始，可为应用程序创建分阶段部署。 通过分阶段部署，可以根据可自定义条件和组进行协调安排，有序推出软件。 例如，将应用程序部署到试点集合，然后根据成功条件自动继续推出。
...
2. 选择其中包含要删除的部署的应用程序或应用程序组。  

3. 切换到细节窗格的“部署”选项卡，然后选择部署  。  

4. 在功能区的“部署”选项卡和“部署”组中，单击“删除”    。  

删除应用程序部署时，不会删除客户端已安装的应用程序的任何实例。 要删除这些应用程序，请将应用程序部署到计算机以“卸载”  。 如果删除应用程序部署，或从部署到的集合中删除资源，则应用程序将不再显示在软件中心中。
...
2. 切换到“安装”行为选项卡，然后单击“添加”   。  

3. 在“添加可执行文件”对话框中，输入目标可执行文件的名称  。 或者，可以为应用程序输入友好名称，以帮助在列表中识别它。  

4. 单击“确定”，然后单击“确定”以关闭部署类型属性窗口   。  
...
<!-- 1322613 -->
如果将应用程序部署为用户可用，用户可以在 Azure Active Directory (Azure AD) 设备上通过软件中心浏览和安装应用程序。  
...
- 将任何应用程序内容分发到[云分发点](../../core/plan-design/hierarchy/use-a-cloud-based-distribution-point.md)  
...
- 若要支持基于 Internet 的客户端：  
    ...
    - [云管理网关](../../core/clients/manage/cmg/plan-cloud-management-gateway.md)  
