...
2. 在“检测规则”对话框中，选择“设置类型”以检测部署类型的状态 ：  
    ...
    - **文件系统**：检测设备上是否存在指定的文件或文件夹。 此检测表示已安装应用程序。 指定以下其他详细信息：  
        ...
        - **此文件或文件夹与 64 位系统中的 32 位应用程序相关联**：客户端首先检查 32 位文件位置中是否存在指定文件或文件夹。 如果未找到此文件或文件夹，则客户端搜索 64 位位置。  
...
继续转到下一部分，了解如何将自定义脚本用作检测方法。 或跳到部署类型对应的[“用户体验”](#bkmk_dt-ux)选项。
...
4. 在“运算符”下拉列表中，选择要使用的运算符。 此运算符将所选的条件与指定的值进行比较。 它可评估用户或设备是否满足安装要求。 可用运算符将因所选条件而异。 使用 `One Of` 运算符时，Values 字段有必须每行输入一个条目的验证。
...
> [!TIP]  
> Windows PowerShell cmdlet **Import-CMApplication** 具有与此过程相同的功能。 有关详细信息，请参阅 [Import-CMApplication](/powershell/module/configurationmanager/import-cmapplication)。  
