...
CMPivot 和 Edge 安装程序通过 Microsoft 代码签名证书进行签名。 如果“受信任的发行者”存储中未列出该证书，则需要添加它。 否则，当 PowerShell 执行策略设置为“AllSigned”时，CMPivot 和 Edge 安装程序不会运行。
