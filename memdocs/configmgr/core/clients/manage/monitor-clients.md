...
|验证客户端服务是否正在运行|启动客户端（SMS 代理主机）服务|没有其他信息|  
|WMI 事件接收器测试。|重新启动客户端服务|检查与 Configuration Manager 相关的 WMI 事件接收器是否丢失|  
|验证 Windows Management Instrumentation (WMI) 服务是否存在|无修正|没有其他信息|  
|验证是否正确安装了客户端|重新安装客户端|没有其他信息|  
|验证反恶意软件服务的启动类型是否为自动|将服务启动类型重置为自动|没有其他信息|  
|验证反恶意软件服务是否正在运行|启动反恶意软件服务|没有其他信息|  
|验证 Windows 更新服务启动类型是自动还是手动|将服务启动类型重置为自动|没有其他信息|  
|验证客户端服务（SMS 代理主机）启动类型是否为自动|将服务启动类型重置为自动|没有其他信息|  
|验证 Windows Management Instrumentation (WMI) 服务是否正在运行。|启动 Windows Management Instrumentation 服务|没有其他信息|  
|验证 Microsoft SQL CE 数据库是否正常|重新安装 Configuration Manager 客户端|没有其他信息|  
|Microsoft 策略平台 WMI 完整性测试|修复 Microsoft 策略平台|没有其他信息|  
|验证 Microsoft 策略平台服务是否存在|修复 Microsoft 策略平台|没有其他信息|  
|验证 Microsoft 策略平台服务启动类型是否为手动|将服务启动类型重置为手动|没有其他信息|  
|验证是否存在后台智能传输服务|无修正|没有其他信息|  
|验证后台智能传输服务启动类型是自动还是手动|将服务启动类型重置为自动|没有其他信息|  
|验证网络检查服务启动类型是否为手动|将服务（如果已安装）启动类型重置为手动|没有其他信息|  
|验证 Windows Management Instrumentation (WMI) 服务启动类型是否为自动|将服务启动类型重置为自动|没有其他信息|  
|验证 Windows 8 设备上的 Windows 更新服务启动类型是自动还是手动|将服务启动类型重置为手动|没有其他信息|  
|请验证客户端（SMS 代理主机）服务是否存在。|无修正|没有其他信息|  
|验证 Configuration Manager 远程控制服务启动类型是自动还是手动|将服务启动类型重置为自动|没有其他信息|  
|验证 Configuration Manager 远程控制服务是否正在运行|启动远程控制服务|没有其他信息|  
