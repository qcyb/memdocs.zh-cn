...
description: 通过 Configuration Manager 中的本地移动设备管理（MDM），创建配置项目来管理 Windows 10 计算机的设置。
ms.date: 01/14/2020
...
- **允许提交诊断数据（Windows 10）**：设置用于分析的 Windows 诊断数据级别： "禁用"、"基本"、"增强" 或 "完整"。 （仅 Windows 10）
...
- **区域设置修改（仅限桌面版）**：允许或禁止用户更改设备上的区域设置。
- **修改电源和睡眠设置（仅限桌面设备）**：允许或禁止用户更改设备上的电源和睡眠设置。
- **修改语言设置（仅限桌面版）**：允许或禁止用户更改设备上的语言设置。
...
- **纯文本电子邮件的最大大小（自动下载）**：在自动下载时设置纯文本电子邮件的最大大小。
- **Html 电子邮件的最大大小（自动下载）**：设置自动下载的 html 电子邮件的最大大小。
- **附件的最大大小（自动下载）**：设置自动下载的电子邮件附件的最大大小。
- **日历同步**：允许或禁止将日历同步到设备。
...
- **更新（windows 10）**：配置 Windows 软件更新行为。 （仅 Windows 10）
...
- **启用网络检查系统**：网络检查系统（NIS）帮助保护设备免遭基于网络的攻击。 它使用 Microsoft Endpoint Protection 中心中的已知漏洞签名来帮助检测和阻止恶意流量。
