...
10. 在“安装应用程序”  页上，指定要安装在目标计算机上的应用程序。 如果指定多个应用程序，你也可以指定任务序列在特定应用程序的安装失败时继续进行。  

11. 完成向导。  

你现在可以将任务序列部署到计算机集合。 有关详细信息，请参阅 [Deploy a task sequence](deploy-a-task-sequence.md)。


## <a name="pre-cache-content"></a>预缓存内容

<!--4224642-->
从版本 1906 开始，可以启用此类型的任务序列来预缓存内容。 对于任务序列的可用部署，可使用预先缓存功能让客户端在用户安装任务序列之前下载相关内容。  

有关详细信息，请参阅[配置预缓存内容](configure-precache-content.md)。


## <a name="example-task-sequence"></a><a name="BKMK_InstallExistingOSImageTSExample"></a> 任务序列示例

使用下表为创建任务序列以部署使用现有映像的 OS 提供指导。 该表将帮助你决定任务序列步骤的常规顺序，以及如何将这些任务序列步骤组织并构建成逻辑组。 你创建的任务序列可能与此示例有所不同，可以包含更多或较少的任务序列步骤和组。  

> [!NOTE]  
> 使用“创建任务序列向导”来创建此任务序列。  
>
> 使用“创建任务序列向导”创建此新任务序列时，如果你手动将这些任务序列步骤添加至现有任务序列，则某些步骤名称可能与应使用的名称不同。

|任务序列组或步骤|说明|  
|---------------------------------|-----------------|  
|捕获文件和设置 -（新建任务序列组） |创建任务序列组。 任务序列组将类似的任务序列步骤放在一起，以获得更佳的组织和错误控制。<br /><br /> 此组包含从引用计算机操作系统捕获文件和设置所需的步骤。|  
|捕获 Windows 设置|使用此任务序列步骤来标识要从引用计算机捕获的 Microsoft Windows 设置。 您可以捕获计算机名称、 用户和组织信息和时区设置。|  
|捕获网络设置|使用此任务序列步骤来从引用计算机捕获网络设置。 您可以捕获引用计算机和网络适配器设置信息的域或工作组成员身份。|  
|捕获用户文件和设置 -（新的任务序列子组） |创建任务序列组内的一个任务序列组。 此子组包含捕获用户状态数据所需的步骤。 与添加的初始组类似，此子组将相似的任务序列步骤保存到一起，以实现更好的组织和错误控制。|  
|请求用户状态存储|使用此任务序列步骤来请求访问状态迁移点存储用户状态数据的位置。 您可以将此任务序列步骤配置为捕获或还原用户状态信息。|  
|捕获用户文件和设置|使用此任务序列步骤以使用用户状态迁移工具 (USMT) 来捕获用户状态和设置从引用计算机将接收与此任务步骤关联的任务序列。 你可以使用标准选项或配置要捕获的选项。|  
...
|应用操作系统|使用此任务序列步骤安装到目标计算机上的操作系统映像。 此步骤先删除卷上的所有文件，但 Configuration Manager 特定的控制文件除外。 然后，它会将 WIM 文件中包含的所有卷映像应用到目标计算机上的相应顺序磁盘卷。 您可以指定 **sysprep** 答案文件，还将配置的磁盘分区用于安装。|  
...
|应用设备驱动程序|使用此任务序列步骤作为操作系统部署的一部分安装驱动程序。 你可以通过选择“考虑所有类别的驱动程序”  允许 Windows 安装程序搜索所有现有的驱动程序类别；或者选择“将驱动程序匹配限制为仅考虑所选类别的驱动程序”  以限制 Windows 安装程序搜索的驱动程序类别。<br /><br /> 此步骤使用只读 **_SMSTSMediaType** 任务序列变量。 此任务序列步骤仅在变量的值不等于 FullMedia  时运行。|  
...
|还原用户文件和设置|使用此任务序列步骤来运行用户状态迁移工具 (USMT) 将用户状态和设置还原到目标计算机。|  
