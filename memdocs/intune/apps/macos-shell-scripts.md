...
ms.locfileid: "83990580"
---
# <a name="use-shell-scripts-on-macos-devices-in-intune"></a>在 Intune 中对 macOS 设备使用 shell 脚本
...
6. 选择“分配” >  选择要包含的组 。 随即显示 Azure AD 组的现有列表。 选择要接收脚本的一个或多个用户或设备组。 选择“选择”。 所选组将显示在列表中，并将收到脚本策略。
   > [!NOTE]
   > - 分配给用户组的 Shell 脚本适用于登录 Mac 的任何用户。  
   > - 如果更新 Shell 脚本的分配，还会更新 [macOS 的 Microsoft Intune MDM 代理](macos-shell-scripts.md#microsoft-intune-management-agent-for-macos)的分配。
...
可能有以下几个原因：
* 代理可能需要签入才能接收新的或更新的脚本。 此签入过程每 8 小时发生一次，且与 MDM 签入不同。 确保设备处于唤醒状态并连接到网络以成功签入代理，并等待代理签入。 你还可以请求最终用户在 Mac 上打开公司门户，选择设备，然后单击“检查设置”。
...
在已安装代理的托管 Mac 上，打开“公司门户”，选择本地设备，然后单击“检查设置” 。 这会启动 MDM 签入以及代理签入。

或者，打开“终端”，运行 `sudo killall IntuneMdmAgent` 命令来终止 `IntuneMdmAgent` 进程。 `IntuneMdmAgent` 进程会立即重启，这会通过 Intune 启动签入。

> [!NOTE]
> Microsoft Endpoint Manager 管理控制台中设备的“同步”操作会启动 MDM 签入，但不会强制执行代理签入。
 ...
 ### <a name="why-are-scripts-running-even-though-the-mac-is-no-longer-managed"></a>为什么即使 Mac 已不受管理，脚本仍会运行？
 当分配有脚本的 Mac 不再受管理时，代理不会立即被删除。 代理在下一次代理签入时（通常每隔 8 小时），检测到 Mac 已不受管理，便会取消计划的脚本运行。 因此，所有存储在本地，且计划比下一次计划的代理签入运行得更频繁的脚本都会运行。 当代理无法签入时，它将重试最多 24 小时（设备唤醒时间），然后将其自身从 Mac 中删除。
