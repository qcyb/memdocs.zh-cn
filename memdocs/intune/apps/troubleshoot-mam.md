...
平台 | 方案 | 说明 |
---| --- | --- |
iOS | 即使将数据传输策略设置为“仅托管应用”  或“无应用”  ，最终用户也可使用 iOS/iPadOS 共享扩展在非托管应用中打开工作或学校数据。 这样不会泄漏数据吗？ | 在不管理设备的情况下，Intune 应用保护策略不能控制 iOS/iPadOS 共享扩展。 因此，**Intune 会在应用外共享“企业”数据前先加密数据**。 可通过尝试在托管应用外打开“企业”文件进行验证。 该文件应进行加密，且无法在托管应用外打开。
