...
> [!NOTE]  
> 在 2019 年 6 月，MDM 安全基线预览版已被 2019 年 5 月版 MDM 安全基线模板所取代，该版本已正式发布（不以预览版提供）。 在 2019 年 5 月版 MDM 安全基线可用之前创建的配置文件不会更新以反映 2019 年 5 月版 MDM 安全基线中的设置和值。  虽然不能基于预览模板创建新的配置文件，但可编辑并继续使用之前基于预览模板创建的配置文件。 
...
有关详细信息，请参阅 Windows 文档 [Policy CSP - CredentialsDelegation](/windows/client-management/mdm/policy-csp-credentialsdelegation
...
有关详细信息，请参阅 Windows 文档 [Policy CSP - DataProtection](/windows/client-management/mdm/policy-csp-dataprotection
...
有关详细信息，请参阅 Windows 文档 [Policy CSP - DeviceGuard](/windows/client-management/mdm/policy-csp-deviceguard
)（策略 CSP - DeviceGuard）。  
...
- **身份验证级别**  
  ...
  - 发送 LM 和 NTLM - 如果进行协商，则发送 NTLMv2 - 客户端使用 LM 和 NTLM 身份验证，并在服务器支持时使用 NTLMv2 会话安全性；域控制器接受 LM、NTLM 和 NTLMv2 身份验证。 
...
- **Defender 可能不需要的应用操作**  
  ...
  [了解详细信息](/windows/client-management/mdm/policy-csp-defender#defender-puaprotection)    
...
有关详细信息，请参阅 Windows 协议文档中的 [2.2.2 FW_PROFILE_TYPOE]( /openspecs/windows_protocols/ms-fasp/7704e238-174d-4a5e-b809-5f3787dd8acc)。  
...
- **阻止 Windows Hello 企业版**  
  ...
  - 若设置为“禁用”，设备将预配 Windows Hello 企业版。
  - 若设置为“未配置”，基线不会影响设备的策略设置。 这意味着设备上 Windows Hello 企业版的当前状态保持不变。

  可在配置 [Windows 注册](windows-hello.md)时或在设备配置文件中禁用 Windows Hello 企业版以实现[标识保护](identity-protection-configure.md)。  

  - **默认值**：Enabled

  此设置设置为“已禁用”时，可使用以下设置。
...
- Internet Explorer Internet 区域自动提示文件下载 > 默认值现为“已禁用”。 在预览版中，此项设置为“已启用”。
