...
- **应用程序类型**：选择应用程序类型。 选项包括：
  ...
  - **添加展台浏览器**：选择“展台浏览器设置”  。 这些设置控制展台上的 Web 浏览器应用。 确保从应用商店获取[展台浏览器应用](https://businessstore.microsoft.com/store/details/kiosk-browser/9NGB5S5XG2KP)，将其作为[客户端应用](../apps/apps-add.md)添加到 Intune。 然后，将应用分配给展台设备。
    ...
    - **允许的网站**：使用此设置允许特定网站打开。 换言之，使用此功能可在设备上限制或阻止网站。 例如，可以允许 `http://contoso.com` 中的所有网站打开。 默认情况下，允许所有网站。
      ...
      要允许特定网站，请以单独的行上传包含允许的网站列表的文件。 如果未添加文件，则允许所有网站。 默认情况下，Intune 支持通配符。 因此，当你输入域（例如 `sharepoint.com`）时，将自动允许使用子域（例如 `contoso.sharepoint.com`、`my.sharepoint.com` 等）。
 ...
 - **针对应用重启指定维护时段**：默认值为“未配置”，选择“需要”以检查需要重启才能完成安装的应用。
 
     如果使用展台浏览器或其他适用于企业的 Microsoft Store 应用，请决定检查需要重启才能完成应用程序安装的应用更新的频率。 如果未配置此设置，则适用于企业的 Microsoft Store 应用将在安装应用更新后的 3 天内的非计划时间重启。
     
     - **维护时段开始时间**：选择日期和时间以开始检查客户端是否需要重启任何应用更新。 默认开始时间为午夜或零分钟。
     
     - **定期维护时段**：默认值为每天。
         设置应用更新的维护时段发生的频率。 建议每天进行一次，以避免计划外的应用重启。
...
- **浏览器和应用程序**：添加要在展台设备上运行的应用。 请记住，可以添加多个应用。
  ...
  - **应用程序**
    ...
    - **添加 Win32 应用**：Win32 应用是一个传统的桌面应用，如 Visual Studio Code 或 Google Chrome。 输入以下属性：
      ...
      - **应用程序名称**：必需。 输入应用程序的名称。
      - **本地路径**：必需。 输入可执行文件的路径，例如 `C:\Program Files (x86)\Microsoft VS Code\Code.exe` 或 `C:\Program Files (x86)\Google\Chrome\Application\chrome.exe`。
    ...
    - **自动登录**：可选。 选择在用户登录时要自动启动的应用程序。 仅自动启动一个应用。
